    ...
    for file in s3.list_objects(Bucket='netgate-transit-vpnconfigs')['Contents']:
        ...
        if vpn_status == 'delete':
            ...
            if ipsecdel.status_code == 200:
                ...
                print("IPSec tunnel{0} has been deleted".format(tunnel_id))
            ...
            if not isinstance(getTunnelId(vpn_gateway_ip), int):
                ...
                requests.put(rest_url+'/route-table-config/static-routes/route-table=ipv4-VRF:0/ipv4-routes/route='+urllib.parse.quote_plus(spoke_subnet), params={}, json=route_table_config, headers=headers)
                print("IPSec tunnel{0} with vpn-getaway:{1} and subnet-{2} has been created".format(tunnel_id, vpn_gateway_ip, spoke_subnet))
